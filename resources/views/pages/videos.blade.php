<x-layouts.app
    title="Dev Videos — Ohene Adjei Effah"
    ...
    <section class="bg-black text-white relative overflow-hidden py-24 md:py-32 border-b border-white/10">
        ...
        <div class="max-w-6xl mx-auto px-7 relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-10">
            ...
            <div class="flex-shrink-0 bg-white/5 border border-white/10 p-5 rounded-[2rem] backdrop-blur-md flex items-center gap-5 hover:bg-white/10 transition-colors group">
                <div class="w-14 h-14 rounded-full bg-[#FF0000] flex items-center justify-center flex-shrink-0 shadow-[0_0_20px_rgba(255,0,0,0.3)] group-hover:scale-105 transition-transform duration-300">
                    <svg class="w-7 h-7 text-white ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                </div>
                <div class="flex flex-col">
                    <h3 class="font-display font-semibold text-white text-lg">Ohene Adjei Effah</h3>
                    <p class="text-sm text-slate-400 mb-3">Tech & Web Development &bull; {{ $videos->count() }} Videos</p>
                    <a href="https://youtube.com/@oheneadjei" target="_blank" rel="noopener" class="inline-flex items-center justify-center gap-2 bg-white text-black hover:bg-slate-200 text-sm font-semibold px-5 py-2 rounded-full transition-colors w-full sm:w-auto shadow-sm">
    ...
    <section class="max-w-6xl mx-auto px-7 py-16">
        @if ($videos->isEmpty())
            <div class="grid md:grid-cols-3 gap-5">
                @for ($i = 0; $i < 3; $i++)
                    <div class="border-2 border-dashed border-black/15 rounded-xl h-40 flex items-center justify-center text-slate-400 font-mono text-xs">Coming soon</div>
                @endfor
            </div>
        @else
